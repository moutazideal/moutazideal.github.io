(() => {
  ...
  $.extend(erpnext.utils, {
    set_party_dashboard_indicators: function(frm) {
      if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
        ...
        if (company_wise_info.length > 1) {
          company_wise_info.forEach(function(info2) {
            erpnext.utils.add_indicator_for_multicompany(frm, info2);
          });
          ...
          var indicator_label = info.balance_label === "Total Advance Paid" ? __("Total Advance Paid: {0}", [format_currency(info.balance_amount, info.currency)]) : info.balance_label === "Total Advance Received" ? __("Total Advance Received: {0}", [
    ...
    add_indicator_for_multicompany: function(frm, info) {
      ...
      var balance_label_text = info.balance_label === "Total Advance Paid" ? __("Total Advance Paid") : info.balance_label === "Total Advance Received" ? __("Total Advance Received") : __("Total Unpaid");
      var indicator = $(
        '<div class="flex-column col-xs-6"><div style="margin-top:10px"><h6>' + info.company + '</h6></div><div class="badge-link small" style="margin-bottom:10px"><span class="indicator blue">Annual Billing: ' + format_currency(info.billing_this_year, info.currency) + '</span></div><div class="badge-link small" style="margin-bottom:10px"><span class="indicator ' + color + '">' + balance_label_text + ": " + format_currency(info.balance_amount, info.currency) + "</span></div></div>"
  ...
  erpnext.utils.get_employee_contact_details = function(frm) {
    if (frm.updating_party_details || frm.doc.party_type !== "Employee")
      return;
    if (!frm.doc.party) {
      reset_contact_fields(frm);
      return;
  ...
  erpnext.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
    ...
    update_bundle_entries() {
      ...
      let total_qty = entries.reduce((total, row) => total + (flt(row.qty) || 1), 0);
  ...
  // frappe-html:/home/fic/frappe-bench/apps/erpnext/erpnext/public/js/templates/visual_plant_floor_template.html
...
`;
  ...
  erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
    ...
    get_item_fields_to_round() {
      ...
      return frappe.meta.get_fieldnames(item.doctype, item.parent, {
      ...
      $.each(this.frm.doc.items || [], function(n, item) {
        ...
        $.each(me.frm.doc["taxes"] || [], function(i, tax) {
          ...
          if (i == 0) {
            ...
            tax.grand_total_fraction_for_current_item = prev.grand_total_fraction_for_current_item + tax.tax_fraction_for_current_item;
    ...
    get_current_tax_fraction(tax, item_tax_map, item) {
      ...
      if (cint(tax.included_in_print_rate)) {
        ...
        if (tax.charge_type == "On Net Total") {
          tax_slope = tax_rate / 100;
        } else if (tax.charge_type == "On Previous Row Amount") {
          const row = this.frm.doc["taxes"][cint(tax.row_id) - 1];
          tax_slope = tax_rate / 100 * row.tax_fraction_for_current_item;
          tax_intercept = tax_rate / 100 * flt(row.inclusive_amount_per_qty);
        } else if (tax.charge_type == "On Previous Row Total") {
          const row = this.frm.doc["taxes"][cint(tax.row_id) - 1];
          tax_slope = tax_rate / 100 * row.grand_total_fraction_for_current_item;
          tax_intercept = tax_rate / 100 * flt(row.grand_total_amount_per_qty);
        } else if (tax.charge_type == "On Item Quantity") {
          tax_intercept = flt(tax_rate);
        } else {
          const qty = flt(item.qty) || 1;
          const base = this.get_item_taxable_base(item, tax);
          tax_intercept = tax_rate / 100 * base / qty;
        }
      }
    ...
    get_item_taxable_base(item, tax) {
      ...
      if (resolver)
        return flt(resolver(this, item, tax));
    ...
    get_current_tax_amount(item, tax, item_tax_map) {
      ...
      if (tax.charge_type == "Actual") {
        ...
        current_tax_amount = tax_rate / 100 * resolved_base;
      }
      if (!tax.dont_recompute_tax) {
    ...
    calculate_outstanding_amount(update_paid_amount) {
      ...
      if (["Sales Invoice", "POS Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
        ...
        if (this.frm.doc.party_account_currency == this.frm.doc.currency) {
          total_amount_to_pay = flt(grand_total - this.frm.doc.total_advance - this.frm.doc.write_off_amount, precision("grand_total"));
        } else {
  ...
  erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
    ...
    setup_quality_inspection() {
      ...
      const inspection_type = incoming_doctypes.includes(this.frm.doc.doctype) || this.frm.doc.doctype === "Stock Entry" && incoming_purposes.includes(this.frm.doc.purpose) ? "Incoming" : "Outgoing";
          ...
          callback: function(r) {
            if (!r.exc) {
              $.each(me.frm.doc.items || [], function(i, item) {
                if (item.name && Object.prototype.hasOwnProperty.call(r.message, item.name) && r.message[item.name].item_tax_template) {
      ...
      frappe.call({
        ...
        callback: function(r) {
          if (r.message.length == 0) {
            ...
            let fieldname = inspection_type === "Incoming" ? "Inspection Required before Purchase" : "Inspection Required before Delivery";
            frappe.msgprint({
  ...
  // frappe-html:/home/fic/frappe-bench/apps/erpnext/erpnext/public/js/templates/item_selector.html
...
`;
  ...
  // frappe-html:/home/fic/frappe-bench/apps/erpnext/erpnext/public/js/templates/item_quick_entry.html
...
`;
  ...
  // frappe-html:/home/fic/frappe-bench/apps/erpnext/erpnext/public/js/templates/call_link.html
...
`;
  ...
  // frappe-html:/home/fic/frappe-bench/apps/erpnext/erpnext/public/js/templates/crm_activities.html
...
.form-footer {
  ...
  // frappe-html:/home/fic/frappe-bench/apps/erpnext/erpnext/public/js/templates/crm_notes.html
      ...
      erpnext.buying.BuyingController = class BuyingController extends erpnext.TransactionController {
        ...
        prevent_past_schedule_dates(frm) {
          ...
          if (frm.doc.transaction_date && frm.fields_dict["schedule_date"]) {
            (_a = frm.fields_dict["schedule_date"].datepicker) == null ? void 0 : _a.update({
...
//# sourceMappingURL=erpnext.bundle.ZO25VGKE.js.map
